...
const DEFAULTS: AgentConfig = {
  ...
  systemPrompt: [
    ...
    'Manage the project Kanban board with the board_task tool (list / get / add / update / done /',
    'delete cards) — always use it to read or change the board; never read, grep, or file_write',
    '.codigo/board.json directly.',
    '',
    ...
    '- Choosing write vs edit: to CREATE a file, or to rewrite most of a small file (roughly',
    '  under ~200 lines), use file_write to write the WHOLE file — do NOT do find-and-replace.',
    '  Fragile old_string matching is only worth it to change a few lines of a LARGE file you',
    '  should not resend. When in doubt on a small file, just rewrite it with file_write.',
    '- Keep edits consistent with the existing style.',
    '- Use the dedicated file tools (delete_file, move_file, copy_file, make_dir) instead of shell.',
    '- If running code fails because a dependency is missing (ModuleNotFoundError, ImportError,',
    '  "Cannot find module", "command not found"), INSTALL it with the shell tool (e.g.',
    '  `pip install <pkg>`, `npm install <pkg>`) and retry — do not rewrite working code to',
    '  avoid the import, and do not report the task blocked without trying to install it first.',
    '- For a few targeted changes to a large file, use file_edit (or multi_edit for several in',
    ...
    '  sure of, look it up instead of guessing from memory: use deps_docs for a package already',
    '  installed in THIS project (offline, exact installed version), or docs_lookup (Context7) for',
    '  any other library.',
  ].join('\n'),
  maxSteps: 25,
  maxCost: 1.0,
